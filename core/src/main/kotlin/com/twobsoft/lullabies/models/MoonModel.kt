...
class MoonModel: Entity() {
    ...
    val luna = LayerActor(tex = lunaTex).also {
        it.actions.add(
            Actions.repeat(
                RepeatAction.FOREVER,
                Actions.parallel(
    ...
    val flag = LayerActor(tex = flagTex).also {
    ...
    override val all = arrayOf<Actor>(background, star1, star2, star3, planets, zemlya, gory, luna, flag)
}