...
class BackgroundSoundService: Service() {
    ...
    override fun onCreate() {
        mediaPlayer = MediaPlayer.create(this, playlist[0].data)
        super.onCreate()
    }
    ...
    fun setIsLooping(value: Boolean) {
        mediaPlayer!!.isLooping = value
        isLooping = value
    }


    fun initEndOfTrackCallback(value: () -> Unit) {
        endOfTrackCallback = value
    }


    fun play(action: String, songIndex: Int) {
        when (action) {
            "playNew" -> {
                mediaPlayer!!.reset()
